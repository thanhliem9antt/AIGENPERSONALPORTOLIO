...
import { changePassword, deleteAccount, login, logout, me, register, updateAccount } from '../controllers/authController.js';
...
router.post('/register', authLimiter, [
  ...
  body('username').trim().matches(/^[a-zA-Z0-9_]+$/).withMessage('Username chỉ gồm chữ, số và dấu gạch dưới'),
  body('email').isEmail().normalizeEmail().withMessage('Email không hợp lệ'),
  passwordRule,
], validate, register);
router.post('/login', authLimiter, [
  body('identity').trim().notEmpty().withMessage('Vui lòng nhập email hoặc username'),
  passwordRule,
], validate, login);
