...
if (!user)
  user = await User.create({ fullName: 'Isaac Reed', username, email: 'demo@example.com', password: 'Demo@123456' });
...
if (!(await PlayedGame.exists({ userId: user.id }))) {
  ...
  await PlayedGame.insertMany(
    demoGames.map(([key, status, hoursPlayed, rank, isFavorite, note], order) => {
...
await Promise.all(
  gameCatalog.map(({ key, title, platform, publisher, genre, coverUrl, gameUrl }) =>
    PlayedGame.updateMany({ gameKey: key }, { title, platform, publisher, genre, coverUrl, gameUrl }),
  ),
);
