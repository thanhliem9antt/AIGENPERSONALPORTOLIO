...
export function EmptyState({ title, description, action }) {
  return (
    <div className="glass grid place-items-center rounded-3xl p-10 text-center">
      <Inbox className="mb-4 text-zinc-500" size={34} />
...
export function ErrorState({ message = 'Không thể tải dữ liệu', onRetry }) {
  return (
    <div role="alert" className="glass grid place-items-center rounded-3xl border border-red-500/20 p-10 text-center">
      <p className="font-semibold text-red-300">{message}</p>
      <p className="mt-2 text-sm text-zinc-500">Kiểm tra kết nối và thử lại sau ít phút.</p>
      {onRetry && (
        <Button type="button" variant="ghost" className="mt-5" onClick={onRetry}>
          Thử lại
        </Button>
      )}
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" onMouseDown={onClose}>
      <div
        ...
        aria-modal="true"
        aria-label={title}
        className="glass max-h-[90vh] w-full max-w-xl overflow-auto rounded-3xl p-6"
