...
export default function OverviewPage() {
  ...
  const load = useCallback(() => {
    setError(null);
    Promise.all([api.get('/profile/me'), api.get('/social-links'), api.get('/projects'), api.get('/games')])
      .then(([p, s, j, g]) =>
        setStats({
          profile: p.data.profile,
          links: s.data.items.length,
          projects: j.data.items.length,
          games: g.data.games.length,
        }),
      )
      .catch(setError);
  }, []);
    ...
    <section>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold">Chào {user.fullName.split(' ').at(-1)}.</h1>
      ...
      <div className="glass mt-6 rounded-3xl p-6">
        <h2 className="text-lg font-semibold">Hoàn thiện profile</h2>
        <p className="mt-2 text-sm text-zinc-400">
          Thêm thông tin, liên kết, game và dự án để profile kể trọn câu chuyện của bạn.
        </p>
        <div className="mt-5 flex flex-wrap gap-2">
          <Link to="/dashboard/profile">
            <Button variant="accent">Chỉnh sửa profile</Button>
          </Link>
          <Link to="/dashboard/games">
            <Button variant="ghost">Thêm game</Button>
          </Link>
          <Link to="/dashboard/projects">
            <Button variant="ghost">Quản lý dự án</Button>
          </Link>
        </div>
      </div>
    </section>
